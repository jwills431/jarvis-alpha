...
- Configured `tts_voice`/`tts_rate` values are authoritative. A browser-saved Speech selection is recorded together with the configured default it was chosen against, and is discarded on the next load once configuration changes or the voice is no longer installed. Before installed options resolve, the client sends no voice and the server uses its configured voice.
...
- One hundred and three Python tests, browser-core logic tests, Python/JavaScript compilation, shell syntax checks, HTTP health, authentication rejection, live inference, local sample transcription, and in-app browser interaction pass.
...
- Speech engines are selected per voice rather than globally: installed macOS voices and a provisioned Piper voice are listed together and switchable without a restart, so they can be compared directly. The optional local neural voice engine (Piper, installed as the `piper-tts` wheel because the archived standalone macOS binaries omit required libraries) is implemented and covered by source-only tests (engine selection, rate-to-length-scale mapping, standard-input synthesis, rendered-audio playback, and supersession of a rendered phrase by a stop or a newer reply). `scripts/setup_piper.sh` installs the pinned engine and voice, `scripts/check_piper.sh` verifies provisioning and measures render latency before the engine is selected, reply text reaches Piper on standard input through `scripts/piper_synthesize.py` because the `piper-tts` command line would place it in process arguments, one resident worker holds the loaded voice so the pause between spoken sentences is synthesis time rather than a repeated model load, and `docs/PIPER_ACCEPTANCE.md` defines the staged on-device checklist. On-device audio acceptance on the target Mac — natural-voice quality, per-phrase playback latency, and interrupt/stop behavior through `afplay` — remains pending. The default built-in `say` engine is unchanged.
