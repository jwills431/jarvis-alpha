...
JARVIS begins speaking at completed sentence or phrase boundaries while each reply is still being generated through a locally installed macOS voice, whether the question was typed or spoken. The default remains `Daniel` at 190 words per minute. **Speech** selects another already-installed English voice and rate, **Voice on** mutes or enables future replies, and **Stop speaking** immediately interrupts the current reply and discards queued speech. For a more natural, British JARVIS-style voice, an optional fully local neural engine (Piper) can be enabled with `"tts_engine": "piper"`; see `docs/TTS.md`.
