...
JARVIS begins speaking at completed sentence or phrase boundaries while each reply is still being generated through a locally installed macOS voice, whether the question was typed or spoken. The voice and rate come from the configured `tts_voice`/`tts_rate` values; the shipped example remains `Daniel` at 190 words per minute. A choice saved in the Speech panel overrides configuration in that browser only until the configured value changes, at which point configuration applies again on the next load. **Speech** selects another already-installed English voice and rate, **Voice on** mutes or enables future replies, and **Stop speaking** immediately interrupts the current reply and discards queued speech. For a more natural, British JARVIS-style voice, an optional fully local neural engine (Piper) can be installed with `scripts/setup_piper.sh`. It then appears in the Speech panel beside the macOS voices, labelled `neural`, and can be compared against them without a restart. See `docs/TTS.md` to install it, `scripts/check_piper.sh` to measure render latency, and `docs/PIPER_ACCEPTANCE.md` for the on-device checklist. Piper is GPL-3.0; see `THIRD_PARTY.md`.
